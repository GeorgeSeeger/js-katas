var lastDigit = function(arr) {
  ...
  var reduceWith = function(twoDigits) {
    return lastDigit(arr.slice(0, arr.length - 2).concat([twoDigits]));
  }
...
var lastTwoDigits = function(base, expo) {
  base = base.toString();
  expo = expo.toString();
  if (expo.length == 1 && base.length == 1) return (base.slice(-2) ** expo).toString().slice(-2);
  if (base === "0") return "0";
  if (base === "1") return "1";  
  ...
  function endsInEvenN(base, expo) {
    ...
    //console.log(ans);
    return ans.toString().slice(-2).padStart(2, "0");
  }
  ...
  switch (base.slice(-1)) {
    ...
    case "5":
      return base[0] % 2 == 1 && expo % 2 == 1 
                    ...
                    : "25";
    case "9":
      return ((base ** (expo % 2)) * endsInOne(base ** 2, parseInt(expo / 2))).toString().slice(-2);
    case "3":
    case "7":
      return ((base ** (expo % 4)) * endsInOne(base ** 4, parseInt(expo / 4))).toString().slice(-2);
    case "2":
    case "4":
    case "6":
    case "8":
...
console.log(lastTwoDigits(2, 0)); //1
console.log(lastTwoDigits(2, 1)); //2
console.log(lastTwoDigits(2, 2)); //4
console.log(lastTwoDigits(2, 4)); //16
console.log(lastTwoDigits(101, 2)); //01
console.log(lastTwoDigits(2, 201)); //52
console.log(lastTwoDigits(2, "01")); //52
console.log(lastTwoDigits(12, "10")); //24
console.log(lastTwoDigits(47, "23")); //23
console.log(lastTwoDigits(390557, "76")); //01

console.log(lastDigit([ 761983,  64283,  787971,  469635,  130031,  390557,  272118,  683154,  426950 ])); //7
console.log(lastDigit([ 2147483647, 2147483647, 2147483647, 2147483647 ])); //9
